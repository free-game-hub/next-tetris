  ...
  const moveBlock = useCallback((direction: 'left' | 'right' | 'down') => {
    ...
    setState(prevState => {
      const newPosition: [number, number] = [
        prevState.currentBlock!.position[0],
        prevState.currentBlock!.position[1]
      ];
      ...
      if (canMove(newPosition, prevState.currentBlock!.shape, prevState.matrix)) {
