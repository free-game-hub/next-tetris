...
export function getDataList(gameDir: string, locale: string): GameScheme[] {
...
export function getDataBySlug(gameDir:string,slug: string, locale: string): GameScheme | undefined {
    const games = getDataList(gameDir, locale);
    ...
    if (game) {
        ...
        if (!game.iframeSrc.startsWith("https://")) {
...
// 添加新方法
export function getGamesByMultipleSlugs(gameDir:string, slugs: string[], locale: string): GameScheme[] {
    const games = getDataList(gameDir, locale);
...
//随机 4 个游戏
export function getRandomDataList(gameDir:string, locale: string, count: number = 4): GameScheme[] {
    const games = getDataList(gameDir,locale);
    //添加过滤 hook
